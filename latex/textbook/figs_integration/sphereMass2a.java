...
public class sphereMass2a extends Template {

     static {templateClass = new sphereMass2a() ; }
...
//          fEnv("useColorPs", "true") ;
          ...
          fEnv("psFillGray", 0.6) ;
          { double[] xlist = {0,0,Math.sqrt(r*r-z1*z1-y2*y2),Math.sqrt(r*r-z1*z1-y1*y1)} ;
            double[] ylist = {y1,y2,        y2,                 y1} ;
            double[] zlist = {z1,z1,       z1,                  z1} ;
            fPolygon3d(xlist, ylist, zlist, 4, FILLED) ; }
          fEnv("psFillGray", 0.7) ;
          { double[] xlist = {Math.sqrt(r*r-z2*z2-y1*y1),Math.sqrt(r*r-z2*z2-y2*y2)
                               ,Math.sqrt(r*r-z1*z1-y2*y2),Math.sqrt(r*r-z1*z1-y1*y1)} ;
            double[] ylist = {y1,y2,        y2,                 y1} ;
            double[] zlist = {z2,z2,       z1,                  z1} ;
            fPolygon3d(xlist, ylist, zlist, 4, FILLED) ; }
...
//               canvas.append( new fPsWrite("[ ] 0 setdash\n")) ;
          ...
          canvas.append( new fPsWrite(Blue+"\n")) ;
               fLine3d(0,y1,z1,   0, y2,z1) ;
               fLine3d(0,y1,z1,   Math.sqrt(r*r-y1*y1-z1*z1), y1,z1) ;
               fLine3d(0,y2,z1,   Math.sqrt(r*r-y2*y2-z1*z1), y2,z1) ;
//               fLine3d(0,y1,z2,   Math.sqrt(r*r-y1*y1-z2*z2), y1,z2) ;
//               fLine3d(0,y2,z2,   Math.sqrt(r*r-y2*y2-z2*z2), y2,z2) ;
               double ry = Math.sqrt(r*r-y1*y1) ;
               fCurve(new circ3d(proj,   0,y1,0, 0,0,ry, ry,0,0),
                      Math.acos(z1/ry)*180/Math.PI,
                      Math.acos(z2/ry)*180/Math.PI,OPEN)  ;
               ry = Math.sqrt(r*r-y2*y2) ;
               fCurve(new circ3d(proj,   0,y2,0, 0,0,ry, ry,0,0),
                      Math.acos(z1/ry)*180/Math.PI,
                      Math.acos(z2/ry)*180/Math.PI,OPEN)  ;
               fCurve(new circ3d(proj,   0,0,z1, r1,0,0, 0,r1,0),
                      Math.asin(y2/r1)*180/Math.PI,
                      Math.asin(y1/r1)*180/Math.PI,OPEN) ;
               fCurve(new circ3d(proj,   0,0,z2, r2,0,0, 0,r2,0),
                      Math.asin(y2/r2)*180/Math.PI,
                      Math.asin(y1/r2)*180/Math.PI,OPEN) ;

          y1 = r1 ;
          y2 = r1-7 ;
          double y12 = 0.5*r2+0.5*y2 ;
          fEnv("psFillGray", 0.6) ;
          { double[] xlist = {0,0,Math.sqrt(r*r-z1*z1-y2*y2),Math.sqrt(r*r-z1*z1-y1*y1)} ;
            double[] ylist = {y1,y2,        y2,                 y1} ;
            double[] zlist = {z1,z1,       z1,                  z1} ;
            fPolygon3d(xlist, ylist, zlist, 4, FILLED) ; }
          fEnv("psFillGray", 0.7) ;
          { double[] xlist = {Math.sqrt(r*r-z2*z2-r2*r2),Math.sqrt(r*r-z2*z2-y12*y12),
                              Math.sqrt(r*r-z2*z2-y2*y2),
                              Math.sqrt(r*r-z1*z1-y2*y2),Math.sqrt(r*r-z1*z1-y1*y1)} ;
            double[] ylist = {r2, y12, y2,        y2,                 y1} ;
            double[] zlist = {z2,  z2, z2,      z1,                  z1} ;
            fPolygon3d(xlist, ylist, zlist, 5, FILLED) ; }
          canvas.append( new fPsWrite(Blue+"\n")) ;
               fLine3d(0,y1,z1,   0, y2,z1) ;
               fLine3d(0,y1,z1,   0, r2,z2) ;
               fLine3d(0,y2,z1,   Math.sqrt(r*r-y2*y2-z1*z1), y2,z1) ;
               ry = Math.sqrt(r*r-y2*y2) ;
               fCurve(new circ3d(proj,   0,y2,0, 0,0,ry, ry,0,0),
                      Math.acos(z1/ry)*180/Math.PI,
...
//          fTeXlabel3d(0,r*xv+2,r*zv, "bl", "$x^2+y^2+z^2=1$") ;    
          ...
          fArrow3d(0,r1+15,z1+10,  1,r1+1,z1+1) ;
          fTeXlabel3d(0,r1+16,z1+12, "cl", "$\\scriptstyle(0,\\sqrt{1-z^2},z)$") ; 
          fArrow3d(r1,-10,z1+10,  r1+1,-1,z1+1) ;
          fTeXlabel3d(r1,-10,z1+10, "br", 
                     "$\\scriptstyle(\\sqrt{1-z^2},0,z)$") ; 
