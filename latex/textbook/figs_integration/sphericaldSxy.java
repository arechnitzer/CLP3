...
public class sphericaldSxy extends Template {

     static {templateClass = new sphericaldSxy() ; }
...
//          x1 = Math.sin(Math.PI*(1+al/180)) ;
          ...
          int nx = 5 ;
          int ny = 5 ;
          double xM = r/(1.1*Math.sqrt(2)) ; double dx = xM/nx ;
          double yM = r/(1.1*Math.sqrt(2)) ; double dy = yM/ny ;
          ...
          fLine3d(0,0,z1, 0,ym,z1) ;
          fLine3d(0,0,z1, xm,0,z1) ;
          fTeXlabel3d(0,0,zm+3, "cb", "$z$") ;
          fTeXlabel3d(0,ym+3,z1, "cl", "$y$") ;
          fTeXlabel3d(xm,-1,z1-1, "tr", "$x$") ;

          fEnv("lineWidth", 1.0) ;
          double x, y, R, th, thRad ;

          canvas.append( new fPsWrite(Blue+"\n")) ;
          x = 0 ;
          for ( int i=0; i <= nx ; i++ ) {
               R = Math.sqrt(r*r-x*x) ;
               thRad = Math.acos(yM/R) ; th = thRad*180/Math.PI ;
               fCurve(new circ3d(proj,   x,0,0, 0,R,0, 0,0,R),
                      th,90,OPEN)  ;
               x += dx ;
          } 
          ...
          for ( int i=0; i <= ny ; i++ ) {
               R = Math.sqrt(r*r-y*y) ;
               thRad = Math.acos(xM/R) ; th = thRad*180/Math.PI ;
               fCurve(new circ3d(proj,   0,y,0, R,0,0, 0,0,R),
                      th,90,OPEN)  ;
               y += dy ;
          } 


          fTeXlabel3d(-1,0.9*yM,0.93*r, "bc", "$z=f(x,y)$") ;
