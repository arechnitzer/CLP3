...
public class merry extends Template {

     static {templateClass = new merry() ; }
...
//          fEnv("lineWidth", 0.5) ;
          ...
          fArrow3d(cx, cy, cz, cx, cy, cz+40) ;
//          fLine3d(cx-r+dx, cy, cz, cx+r-dx, cy, cz) ;
          fTeXlabel3d(cx,cy,cz+41, "bc", "$\\vOmega$") ;
          
          th = 60*Math.PI/180.0 ;
//          dth = 3*Math.PI/180.0 ;
//          fArrow3d(cx+r*Math.cos(th), cy+r*Math.sin(th), cz,
//                   cx+r*Math.cos(th+dth), cy+r*Math.sin(th+dth), cz) ;
          fLine3d(cx, cy, cz, cx+r*Math.cos(th), cy+r*Math.sin(th), cz) ;
          fTeXlabel3d(cx+1.2*r*Math.cos(th), cy+1.2*r*Math.sin(th), cz, "tc", "$x$") ;
          th = (60+90)*Math.PI/180.0 ;
//          fArrow3d(cx+r*Math.cos(th), cy+r*Math.sin(th), cz,
//                   cx+r*Math.cos(th+dth), cy+r*Math.sin(th+dth), cz) ;
          fLine3d(cx, cy, cz, cx+r*Math.cos(th), cy+r*Math.sin(th), cz) ;
          fTeXlabel3d(cx+1.1*r*Math.cos(th), cy+1.1*r*Math.sin(th), cz, "bl", "$y$") ;
          fTeXlabel3d(cx, cy, cz-15, "tc", "moving observer") ;
