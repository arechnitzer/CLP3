...
\subsection{Vertical Slices}\label{sec_vertical_slices}
...
\begin{itemize}
  ...
  \item
   ...
   techniques.
  \item
    Beware that, in this notation, the inside integral sign provides the limits
    of integration, in this example $B(x)$ and $T(x)$, for the inside 
    integration variable, in this example, the $y$ of $\dee{y}$. The outside 
    integral sign provides the limits of integration, in this example $a$, $b$, 
    for the outside integration variable, in this example, the $x$ of $\dee{x}$. 
\end{itemize}
...
\begin{itemize}
  ...
  \item
  Beware that, in this notation, each integral sign provides the limits
    of integration for the integration variable directly beside it. In this
    example, the limits of integration for $x$ are $a$ and $b$, and the limits
    of integration for $y$ are $B(x)$ and $T(x)$.
  
\end{itemize}

\item
Notation \ref{notn dbl int} provides several different styles of notation 
for the same integral. All are used ``in the wild\footnote{Though mathematicians 
do enjoy getting out into the wilderness as much as anyone, they do not regularly compute multivariate integrals while far from blackboards and civilisation. You will find that practitioners of calculus use a variety of different notations depending on the problem, the context, the conventions of the area, their education, and so on. Accordingly, we recommend that you become comfortable with all of these notations.}'' 
and you should make yourself familar with all of them.
...
\subsection{Horizontal Slices}\label{sec_horizontal_slices}
...
We follow essentially the same procedure as we used 
...
\soln
Our first job is to figure out what the specified solid looks like. 
Note that
\begin{itemize}
\item
...
are symmetric about the $yz$-plane. In particular the volume of the
...
formula\footnote{We weren't joking about this being a good review of
...
in \S\eref{CLP101}{sec:evenodd} of the CLP-2 text, we gave the
\begin{defn}[Definition  \eref{CLP101}{def:INTevenodd} in the CLP-2 text]\label{def_evenodd_one_variable}
...
\begin{theorem}[Theorem \eref{CLP101}{thm:INTevenodd} in the CLP-2 text]
...
We will now see that we can similarly exploit evenness and oddness of functions
...
Definition~\ref{def_evenodd_one_variable} (Definition  \eref{CLP101}{def:INTevenodd} in the CLP-2 text) of evenness and oddness of the function $f(x)$, we compared the value of $f$ at $x$ with the value of $f$ at $-x$. The points $x$ and $-x$ are the same distance from the origin, $0$, and
...
\begin{defn}\label{def_evenodd_two_variables}
...
Recall from  Theorem~\ref{thm:INTevenodd} (or Theorem~\eref{CLP101}{thm:INTevenodd} in the CLP-2 text) that we can exploit the evenness or oddness of the integrand, $f(x)$, 
...
We are finally ready for the analog of Theorem~\ref{thm:INTevenodd}
(Theorem \eref{CLP101}{thm:INTevenodd} in the CLP-2 text)
...
\begin{efig}
\begin{center}
   \includegraphics{oddSymmetric.pdf}
\end{center}
\end{efig}
}
...
\begin{theorem}[2d Even and Odd]\label{thm:INTevenodd2d}
\begin{enumerate}[(a)]
\item 
...
\end{equation*}
by part (b) of Theorem \ref{thm:INTevenodd} (Theorem \eref{CLP101}{thm:INTevenodd} in the CLP-2 text).
\item
If $f(x,y)$ is even under $x\rightarrow -x$, then
$f(-x,y)=f(x,y)$ for all $-R(y)\le x\le R(y)$ and
\begin{equation*}
...
\begin{eg}\label{eg_odd_integralA}
...
\end{equation*}
by part (b) of Theorem \ref{thm:INTevenodd2d} (Theorem \eref{CLP101}{thm:INTevenodd} in the CLP-2 text).
\end{eg}

\begin{eg}\label{eg_odd_integralB}
...
rhodonea\footnote{The name rhodonea first appeared in the 1728 publication
...
However we can evaluate its square
\begin{align*}
...
how to do that derivation more rigorously.
...
=\frac{2}{3}\, a
\end{equation*}
Notice that this number is bigger than zero and less than the maximum height,
...
\begin{eg}[Compound bodies]\label{eg_dblInt_K}
In \S \eref{CLP101}{sec com} of the CLP-2 text, we saw that it was possible
...
\begin{efig}
\begin{center}
   \includegraphics{dumbbell.pdf}\qquad\qquad
\end{center}
\end{efig}
}
...
\begin{eg}[Example \eref{CLP101}{eg:TRQcentroidBb} of CLP-2 revisited]
                                                \label{eg_dblInt_L}
In Example \eref{CLP101}{eg:TRQcentroidBb} of the CLP-2 text, we found the 
...
\begin{efig}
\begin{center}
    \includegraphics{PSIVcb}
\end{center}
\end{efig}
}
\begin{itemize}\itemindent-0.1in \itemsep1pt \parskip0pt \parsep0pt
\item 
...
\end{align*}
which is exactly the answer that we found in Example \eref{CLP101}{eg:TRQcentroidBb} of the CLP-2 text.
...
\end{equation*}
is called the \emph{moment of inertia} of $\cR$ about the axis $\cA$.
...
\begin{eg}[Cardioid]\label{eg moment cardioid}
Find the moment of inertia of the interior, $\cR$,
of the cardioid $r=a(1+\cos\theta)$ about the $z$-axis. 
Assume that the cardioid lies in the $xy$-plane and  has density one.

\soln We sketched the cardioid (with $a=1$) in Example \ref{eg cardioid}.
\vadjust{
\begin{efig}
\begin{center}
...
Find the moment of inertia of the interior, $\cR$,
of the cardioid $r=a(1+\cos\theta)$
...
\end{center}
\end{nfig}
Concentrate on any one of the tiny pieces. Here is a greatly magnified
...
that is in the first octant, which is
\begin{equation*}
...
\item
To get the mass of any one wedge, say the wedge whose $\theta$ coordinate runs 
...
$z=\sqrt{x^2+y^2}\ \cot\varphi$ with $\varphi$ a constant 
(which looks like a conical funnel).
\end{itemize}
\begin{wfig}
\begin{center}
...
Thus these four edges of the cube are circular arcs of radius 
...
\item
To get the volume of any one searchlight, say the searchlight whose 
...
by inserting many cones of constant $\varphi$, with the various values of 
$\varphi$ differing by $\dee{\varphi}$. The figure below shows 
one searchlight outlined in blue. Each searchlight
\vspace{-\topsep}
\begin{itemize} \itemsep1pt \parskip0pt \parsep0pt
\item
