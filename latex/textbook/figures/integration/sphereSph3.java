...
public class sphereSph3 extends Template {

     static {templateClass = new sphereSph3() ; }
...
//          x1 = Math.sin(Math.PI*(1+al/180)) ;
          ...
          double xh, yh, xv, zv, th, phi, X, Y, Z, R ;
...
//          fCurve(new circ3d(proj,   0,0,r*zv, r*xv),2*dth,3*dth,OPEN)  ;
//               phi += dphi ;
//          X = r*xv*Math.cos(th1*Math.PI/180) ;
//          Y = r*xv*Math.sin(th1*Math.PI/180) ;
//          Z = r*zv ;
//          fLine3d(0,0,0, X,Y,Z) ;
//          X = r*xv*Math.cos(th2*Math.PI/180) ;
//          Y = r*xv*Math.sin(th2*Math.PI/180) ;
//          Z = r*zv ;
//          fLine3d(0,0,0, X,Y,Z) ;
//          } 
...
//          xv = Math.sin(phi*Math.PI/180);
//          zv = Math.cos(phi*Math.PI/180);
//          fCurve(new circ3d(proj,   0,0,r*zv, r*xv),1*dth,4*dth,OPEN)  ;

          canvas.append( new fPsWrite("0.0 setgray\n")) ;
          xh = Math.cos(th2*Math.PI/180);
          yh = Math.sin(th2*Math.PI/180);
          double phim = phi2 ;
          double cosphi = Math.cos(phim*Math.PI/180);
          double sinphi = Math.sin(phim*Math.PI/180);
          R = 2.3*dr ;
          fArrow3d(R*sinphi*xh, R*sinphi*yh+20, R*cosphi, 
                   R*sinphi*xh, R*sinphi*yh+1, R*cosphi) ;
          fTeXlabel3d(R*sinphi*xh, R*sinphi*yh+21.5, R*cosphi+0.3, "cl",
                     "$\\dee{\\rho}$") ; 
...
//                 0,90,OPEN)  ;
          ...
          R = 3*dr ;
               ...
               xv = Math.sin(phi1*Math.PI/180);
               zv = Math.cos(phi1*Math.PI/180);
               X = xv*Math.cos(th1*Math.PI/180) ;
               Y = xv*Math.sin(th1*Math.PI/180) ;
               Z = zv ;
               fLine3d(2*dr*X,2*dr*Y,2*dr*Z, 3*dr*X,3*dr*Y,3*dr*Z) ;
               X = xv*Math.cos(th2*Math.PI/180) ;
               Y = xv*Math.sin(th2*Math.PI/180) ;
               fLine3d(2*dr*X,2*dr*Y,2*dr*Z, 3*dr*X,3*dr*Y,3*dr*Z) ;
...
//          canvas.append( new fPsWrite(Red+"\n")) ;
//          xv = Math.sin(90*Math.PI/180);
//          zv = Math.cos(90*Math.PI/180);
