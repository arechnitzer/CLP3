...
public class appleCore3 extends Template {

     static {templateClass = new appleCore3() ; }
...
//          x1 = Math.sin(Math.PI*(1+al/180)) ;
          ...
          double th2 = dth*3 ;
          double phi = dphi*2.5 ;
          double rb = r*Math.sin(phi*Math.PI/180) ;
          double r1 = 0.7*r*Math.sin(phi*Math.PI/180) ;
          double r2 = 0.5*r*Math.sin(phi*Math.PI/180) ;
          double zh1 = Math.sqrt(r*r-r1*r1) ;
          double zh2 = Math.sqrt(r*r-r2*r2) ;
          double phi1 = Math.asin(r1/r) ;
          double phi2 = Math.asin(r2/r) ;
...
//          fCurve(new circ3d(proj,   0,0,r*zv, r*xv),0,90,OPEN)  ;
          ...
          double  xh1 = Math.cos(th1*Math.PI/180) ;
          double  yh1 = Math.sin(th1*Math.PI/180) ;
          double  xh2 = Math.cos(th2*Math.PI/180) ;
          double  yh2 = Math.sin(th2*Math.PI/180) ;
          double  thm = 0.5*th1 + 0.5*th2 ;
          double  xhm = Math.cos(thm*Math.PI/180) ;
          double  yhm = Math.sin(thm*Math.PI/180) ;
          {  double[] xlist = {0,rb*xh1,rb*xhm, rb*xh2} ;
             double[] ylist = {0,rb*yh1,rb*yhm, rb*yh2} ;
             double[] zlist = {0,   0,       0,    0} ;
             fPolygon3d(xlist, ylist, zlist, 4, FILLED) ; }

          fEnv("psFillGray", 0.0) ;
          fCurve(new circ3d(proj,   0,0,0   , r1*xv),th1,th2,FILLED)  ;
          {  double[] xlist = {0,r1*xh1,r1*xhm, r1*xh2} ;
             double[] ylist = {0,r1*yh1,r1*yhm, r1*yh2} ;
             double[] zlist = {0,   0,       0,    0} ;
             fPolygon3d(xlist, ylist, zlist, 4, FILLED) ; }

          fEnv("psFillGray", 0.7) ;
          fCurve(new circ3d(proj,   0,0,0   , r2*xv),th1,th2,FILLED)  ;
          {  double[] xlist = {0,r2*xh1,r2*xhm, r2*xh2} ;
             double[] ylist = {0,r2*yh1,r2*yhm, r2*yh2} ;
             double[] zlist = {0,   0,       0,    0} ;
             fPolygon3d(xlist, ylist, zlist, 4, FILLED) ; }
          ...
          fEnv("lineWidth", 1.0) ;
               fLine3d(r1*xh1,r1*yh1,zh1, r1*xh1,r1*yh1,0) ;
               fLine3d(r1*xh2,r1*yh2,zh1, r1*xh2,r1*yh2,0) ;
               fCurve(new circ3d(proj,   0,0,0, 0,0,r, r*xh1,r*yh1,0),
                      phi1*180/Math.PI,phi2*180/Math.PI,OPEN)  ;

               canvas.append( new fPsWrite("[2.5 2.5] 1.0 setdash\n")) ;
               fLine3d(r2*xh2,r2*yh2,zh2, r2*xh2,r2*yh2,0) ;
               canvas.append( new fPsWrite("[  ] 0 setdash\n")) ;
               fCurve(new circ3d(proj,   0,0,zh1, r1),th1,th2,OPEN)  ;
               fCurve(new circ3d(proj,   0,0,zh2, r2),th1,th2,OPEN)  ;
               fLine3d(r2*xh1,r2*yh1,zh2, r2*xh1,r2*yh1,0) ;
               fCurve(new circ3d(proj,   0,0,0, 0,0,r, r*xh2,r*yh2,0),
                      phi1*180/Math.PI,phi2*180/Math.PI,OPEN)  ;
...
//          fTeXlabel3d(0.5*r*xv/sq2,0.5*r*xv/sq2,0, "cc", "$\\cR$") ;
          ...
          fTeXlabel3d(0,r*xv,r*zv, "bl", "$r^2+z^2=a^2$") ;
