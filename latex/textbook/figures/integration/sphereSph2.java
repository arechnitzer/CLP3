...
public class sphereSph2 extends Template {

     static {templateClass = new sphereSph2() ; }
...
//          x1 = Math.sin(Math.PI*(1+al/180)) ;
          ...
          double xh, yh, xv, zv, th, phi, X, Y, Z ;
...
//          fCurve(new circ3d(proj,   0,0,r*zv, r*xv),1*dth,4*dth,OPEN)  ;
          ...
          fCurve(new circ3d(proj,   0,0,0, 0,0,0.4*r, 0.4*r*xh,0.4*r*yh,0),
                 phi1,phi2,OPEN)  ;
          double phim =0.7*phi1 +0.3*phi2 ;
          double cosphi = Math.cos(phim*Math.PI/180);
          double sinphi = Math.sin(phim*Math.PI/180);
          fArrow3d(1.3*r*sinphi*xh, 1.3*r*sinphi*yh, 1.3*r*cosphi, 
                   0.43*r*sinphi*xh, 0.43*r*sinphi*yh, 0.43*r*cosphi) ;
          fTeXlabel3d(1.3*r*sinphi*xh, 1.3*r*sinphi*yh+1, 1.3*r*cosphi, "cb",
                     "$\\dee{\\varphi}$") ; 
