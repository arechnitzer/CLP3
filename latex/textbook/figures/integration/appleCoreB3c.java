                  /*  File:  appleCoreB3c.java    */


import figPac.* ;
import java.applet.* ;
import java.awt.* ;


public class appleCoreB3c extends Template {

     static {templateClass = new appleCoreB3c() ; }

     public void setup() {
          filePrefix = "appleCoreB3c" ;   
          outputPsFile = true ;
          outputLblFile = true ;
          xsize = 7*0.4 ;      // in inches
          ysize = 3.5*0.4 ;   // in inches
          xmin = -1 ;
          xmax = 6 ;
          ymin = -1 ;
          ymax = 2.5 ;
          topmargin = 0.1 ;   // in inches
          bottommargin = 0 ;   // in inches
          leftmargin = 0 ;   // in inches
          rightmargin = 0 ;   // in inches
          useZoom = true ;
          useDrag = true ;
          showCoords = true ;
          showCanvasBoundary = true ;   
     }
     
     public  void prepareFigure(Figure canvas) {


          fEnv("lineWidth", 0.5) ;
          fTeXlabel(-0.07, ymax, "tr", "$z$") ;
          double beta = 70 ; double betaRad = beta*Math.PI/180 ;
          double a = 5 ;  
          double b = a*Math.cos(betaRad) ;

          double n = 6 ;
          double rn = 5 ;
          double dth  = beta/n ;
          double th2 = 2*dth ; double th2Rad = th2*Math.PI/180 ;
          double th1 = 1*dth ; double th1Rad = th1*Math.PI/180 ;
          double rmin = b/Math.cos(th1Rad) ;
          double dr = (a-rmin)/rn ;
          double r1 = rmin  ;
          double r2 = rmin + dr ;
          fEnv("psFillGray", 0.9) ;
          fEllipticalArc(0,0, a,a, 0, beta, WEDGE+FILLED) ;

          fEnv("psFillGray", 0.7) ;
          fEllipticalArc(0,0, a,a, th1, th2, WEDGE+FILLED) ;

          fEnv("psFillGray", 0.0) ;
          fEllipticalArc(0,0, r2,r2, th1, th2, WEDGE+FILLED) ;

          fEnv("psFillGray", 0.7) ;
          fEllipticalArc(0,0, r1,r1, th1, th2, WEDGE+FILLED) ;


          fEnv("psFillGray", 1.0) ;
          { 
            double[] xlist = { 0, b, b, 0} ;
            double[] ylist = { 0, 0, a, a} ;
            fPolygon(xlist, ylist, 4, FILLED) ;
           }             


          fEnv("psFillGray", 1.0) ;
          fEnv("lineWidth", 0.5) ;
          double th = th2 ;  double xv, yv ;
          for ( int i=1; i <= 1 ; i++ ) {
               xv = 6*Math.cos(th*Math.PI/180);
               yv = 6*Math.sin(th*Math.PI/180);
               fLine(0,0,xv,yv) ;
               th += dth ;
          } 

          double r = rmin+dr ;  
          for ( int i=1; i <= rn ; i++ ) {
          fEllipticalArc(0,0, r,r,  th1, th2, OPEN) ;
               r += dr ;
          } 

          double rr = 0.3 ;
          fEllipticalArc(0,0, rr,rr,  th2, 90, OPEN) ;
          fTeXlabel(0.6*rr, 0.9*rr, "bl",   "$\\scriptstyle\\varphi$") ;
          fTeXlabel(0.5*b,  -0.1, "tc",    "$b$") ;
          fTeXlabel(0.65*b,  0.15, "bc",    "$\\rho$") ;



          fLine(xmin, 0, xmax, 0) ;
          fLine(0, ymin, 0, ymax) ;
          double h = b*Math.tan(th2Rad) ;
          fLine(0,h,  b,h) ;
          fTeXlabel(0.5*b,  h+0.1, "bc",    "$b$") ;


          String Red = "0 1 1 0 setcmykcolor  " ;
          String Blue = "1 1 0 0 setcmykcolor  " ;
          String Black = "0 0 0 1 setcmykcolor  " ;
          canvas.append( new fPsWrite(Red+"\n")) ;
          fEnv("lineWidth", 1.0) ;
          fEllipticalArc(0,0, a,a, 0, beta, OPEN) ;
          fLine(b,0,  b,a*Math.sin(betaRad)) ;
          fLine(b,0,  a,0) ;

          fEnv("lineWidth", 1.5) ;
          canvas.append( new fPsWrite(Blue+"\n")) ;
          fEllipticalArc(0,0, a,a,  th1, th2, OPEN) ;

          fLine(b,b*Math.tan(th1Rad), a*Math.cos(th1Rad),a*Math.sin(th1Rad)) ;
          fLine(b,b*Math.tan(th2Rad), a*Math.cos(th2Rad),a*Math.sin(th2Rad)) ;
          fLine(b,b*Math.tan(th1Rad), b,b*Math.tan(th2Rad)) ;


//          fTeXlabel(6*Math.cos(betaRad)+0.1, 6*Math.sin(betaRad), "bl", 
//                                              "$\\varphi=\\arcsin\\frac{b}{a}$") ;
//          fTeXlabel(6.1*Math.cos(th1Rad),  6.1*Math.sin(th1Rad), "bl", 
//                                          "$\\scriptstyle\\varphi+\\dee{\\varphi}$") ;
//          fTeXlabel(0.5*b-0.05,0.5*a*Math.sin(betaRad), "br",    "$a$") ;

     }
}
