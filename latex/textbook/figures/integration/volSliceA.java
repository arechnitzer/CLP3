...
public class volSliceA extends Template {

     static {templateClass = new volSliceA() ; }
...
//          x1 = Math.sin(Math.PI*(1+al/180)) ;
          ...
          fEnv("psFillGray", 0.9) ;
          fCurve(new circ3d(proj,   0,0,0, r),0,90,FILLED)  ;
          {  double[] xlist = {0,r,  r/Math.sqrt(2) , 0} ;
             double[] ylist = {0,0,  r/Math.sqrt(2), r} ;
          ...
          fTeXlabel3d(0,ym/2.0,zm-12, "tl", "$z=f(x,y)$") ;
...
//          fCurve(new circ3d(proj,   0,0,r*zv, r*xv),0,360,OPEN)  ;
          fCurve(new circ3d(proj,   0,0,r*zv, r*xv),0,90,OPEN)  ;

          canvas.append( new fPsWrite("0.6 setgray\n")) ;
          fEnv("psFillGray", 0.7) ;
          double Y1 = 0.5*r ;  double rr1 = Math.sqrt(r*r-Y1*Y1) ;
          double Y2 = 0.6*r ;  double rr2 = Math.sqrt(r*r-Y2*Y2) ;
          double X1 = 0.2*r ;  double X2 = 0.4*r ;
          {  double[] xlist = { 0, 0, 0.5*rr1+0.5*rr2, 0.5*rr1+0.5*rr2} ;
          ...
          canvas.append( new fPsWrite("0.6 setgray\n")) ;
//          fLine3d(rr1,Y1,0, -rr1,Y1,0) ;
          fLine3d(0.5*rr1+0.5*rr2,Y1,0, 0,Y1,0) ;
//          fLine3d(0,Y1,0, 0,Y1,rr1) ;
//          fLine3d(rr,Y2,0, -rr2,Y2,0) ;
          fLine3d(0.5*rr1+0.5*rr2,Y2,0, 0,Y2,0) ;
          fLine3d(0.5*rr1+0.5*rr2,Y2,0, 0.5*rr1+0.5*rr2,Y1,0) ;
          fLine3d(0,Y2,0, 0,Y1,0) ;
//          fLine3d(0,Y2,0, 0,Y2,rr2) ;



          fEnv("lineWidth", 0.8) ;
          canvas.append( new fPsWrite("0.2 setgray\n")) ;
          fLine3d(X1, Y1, 0, X1, Y2, 0) ;
          fLine3d(X2, Y1, 0, X2, Y2, 0) ;
          fLine3d(X1, Y1, 0, X2, Y1, 0) ;
          fLine3d(X1, Y2, 0, X2, Y2, 0) ;
          double Z11 = Math.sqrt(r*r-X1*X1-Y1*Y1) ;
          double Z12 = Math.sqrt(r*r-X1*X1-Y2*Y2) ;
          double Z21 = Math.sqrt(r*r-X2*X2-Y1*Y1) ;
          double Z22 = Math.sqrt(r*r-X2*X2-Y2*Y2) ;
          fLine3d(X1, Y2, 0, X1, Y2, Z12) ;
          fLine3d(X2, Y1, 0, X2, Y1, Z21) ;
          fLine3d(X2, Y2, 0, X2, Y2, Z22) ;
          fLine3d(X1, Y1, Z11, X1, Y2, Z12) ;
          fLine3d(X2, Y1, Z21, X2, Y2, Z22) ;
          fLine3d(X1, Y1, Z11, X2, Y1, Z21) ;
          fLine3d(X1, Y2, Z12, X2, Y2, Z22) ;
          canvas.append( new fPsWrite("[3 3] 0 setdash\n")) ;
          fLine3d(X1, Y1, 0, X1, Y1, Z11) ;
          canvas.append( new fPsWrite("[ ] 0 setdash\n")) ;

          canvas.append( new fPsWrite("0.6 setgray\n")) ;
//          fCurve(new circ3d(proj,   0,Y1,0, 0,0,rr1, rr1,0,0),
//                       0,90,OPEN)  ;
//          fCurve(new circ3d(proj,   0,Y2,0, 0,0,rr2, rr2,0,0),
//                       0,90,OPEN)  ;
