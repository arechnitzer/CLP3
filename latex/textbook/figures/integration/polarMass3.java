...
public class polarMass3 extends Template {

     static {templateClass = new polarMass3() ; }

     public void setup() {
          filePrefix = "polarMass3" ;   
          outputPsFile = true ;
          outputLblFile = true ;
          xsize = 1.8 ;      // in inches
          ysize = 1.8 ;   // in inches
...
//          double th2 = a+ 2*dth ;
          ...
          double phi1Deg = 40 ;  double phi1 = phi1Deg*Math.PI/180.0 ;
          double phi2Deg = 50 ;  double phi2 = phi2Deg*Math.PI/180.0 ;
          ...
          double r1 = 2.5 ;
          double r2 = 3 ;
          double rm = 0.5*r1+0.5*r2 ;
          double XX1 = r1*Math.cos(phi1);
          double YY1 = r1*Math.sin(phi1);
          double XX2 = r2*Math.cos(phi2);
          double YY2 = r2*Math.sin(phi2);
//          double Xm = (3+2*Math.sin(phim))*Math.cos(phim);
//          double Ym = (3+2*Math.sin(phim))*Math.sin(phim);
          fEnv("psFillGray", 0.0) ;
//          {double[] xlist = {0, XX1, XXm, XX2} ;
//           double[] ylist = {0, YY1, YYm, YY2} ;
//           fPolygon(xlist, ylist, 4, FILLED) ;}
          fEllipticalArc(0,0, r2,r2, phi1Deg, phi2Deg, FILLED+WEDGE);
          fEllipticalArc(0,0, r2,r2, 7, 105, OPEN);
          fEnv("psFillGray", 0.7) ;
          fEllipticalArc(0,0, r1,r1, phi1Deg, phi2Deg, FILLED+WEDGE);
          fEllipticalArc(0,0, r1,r1, 7, 105, OPEN);
...
//                                                             "$\\cR$") ;
          fTeXlabel(0.5*(r1+r2)*Math.cos(0.53*Math.PI),0.5*(r1+r2)*Math.sin(0.53*Math.PI),
                                  "cr", "$\\scriptstyle\\dee{r}$") ;
