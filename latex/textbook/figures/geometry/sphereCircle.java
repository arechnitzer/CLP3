...
public class sphereCircle extends Template {

     static {templateClass = new sphereCircle() ; }
...
//          x1 = Math.sin(Math.PI*(1+al/180)) ;
          ...
          fEnv("lineWidth", 0.5) ;
//          fCurve(new circ3d(proj,   A,B,C, 0,R,0, 0,0,R),
//                 0,360,CLOSED)  ;
//          fCurve(new circ3d(proj,   A,B,C, R,0,0, 0,0,R),
//                 0,360,CLOSED)  ;
          T1 = 180*Math.asin(B/R)/Math.PI ;
          T2 = 180*Math.asin(A/R)/Math.PI ;
//          fCurve(new circ3d(proj,   A,B,C, R,0,0, 0,R,0),
//                 -T1,90+T2,OPEN)  ;
//          fCurve(new circ3d(proj,   A,B,C, R,0,0, 0,R,0),
//                 270-T2,180+T1,OPEN)  ;
//          fCurve(new circ3d(proj,   A,B,C, R,0,0, 0,R,0),
...
//                 -T1,180+T1,OPEN)  ;
          ...
          th = 0 ;
          T1 = 180*Math.asin(C/R)/Math.PI ;
          for ( int i=0; i <= nth ; i++ ) {
               xh = Math.cos(th*Math.PI/180);
               yh = Math.sin(th*Math.PI/180);
//               fCurve(new circ3d(proj,   A,B,C, R*xh,R*yh,0, 0,0,R),
//                      -T1,180+T1,OPEN)  ;
               fCurve(new circ3d(proj,   A,B,C, R*xh,R*yh,0, 0,0,R),
                      -T1,90,OPEN)  ;
               th += dth ;
          ...
          fCurve(new circ3d(proj,   0,0,C+R*zv, R*xv),0,90,OPEN)  ;
               phi += dphi ;
          } 
          ...
          fEnv("lineWidth", 2.0) ;
//          r = Math.sqrt(R*R-A*A) ;
//          fCurve(new circ3d(proj,   0,B,C, 0,r,0, 0,0,r),
//                 0,360,CLOSED)  ;
//          r = Math.sqrt(R*R-B*B) ;
//          fCurve(new circ3d(proj,   A,0,C, r,0,0, 0,0,r),
//                 0,360,CLOSED)  ;
          r = Math.sqrt(R*R-C*C) ;
          fCurve(new circ3d(proj,   A,B,0, r,0,0, 0,r,0),
                 0,90,OPEN)  ;
