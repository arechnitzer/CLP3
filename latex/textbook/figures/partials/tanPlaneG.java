...
public class tanPlaneG extends Template {

     static {templateClass = new tanPlaneG() ; }
...
//          x1 = Math.sin(Math.PI*(1+al/180)) ;
          ...
          fCurve(new circ3d(proj,   0,0,0, rx,0,0, 0,0,rz),
                      0,90,OPEN)  ;
          fTeXlabel3d(0.5*rx,1,0.95*rz, "br", "$G(x,y,z)=0$") ;
          ...
          double XX = 80 ; double ZZ = Z*Math.sqrt(1-XX*XX/(X*X)) ;
          double XB = 80 ; double ZB = rz*Math.sqrt(1-XB*XB/(rx*rx)) ;
                          double YB = ry*Math.sqrt(1-XB*XB/(rx*rx)) ;
          double XC = 80 ; double YC = 40 ;
                           double ZC = rz*Math.sqrt(1-XC*XC/(rx*rx)-YC*YC/(ry*ry)) ;
...
//                      20,80,OPEN)  ;
          ...
          fArrow3d(XX,2.9*Y,ZZ+30,  XX, Y+2 ,ZZ) ;
          fTeXlabel3d(XX,2.9*Y+3,ZZ+30, "cl", "$\\vr(0)=\\big(x_0,y_0,z_0\\big)$") ;
          fDisk3d(XC,YC,ZC, r, r*aspect) ;
          fArrow3d(XX,2.9*Y,ZZ,  XC, YC+2 ,ZC) ;
          fTeXlabel3d(XX,2.9*Y+3,ZZ, "cl", "$\\vr(t)$") ;
