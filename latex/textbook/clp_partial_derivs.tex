...
\item[$\circ$]
$m$ and $n$ be natural numbers\footnote{In this text, we will be interested in
...
Partial derivatives are used a lot. 
%lot\footnote{There is a definition of ``$f(x,y)$
%is differentiable at $(x_0,y_0)$'' that is more restrictive than ``the partial 
%derivatives $\pdiff{f}{x}(x_0,y_0)$ and $\pdiff{f}{y}(x_0,y_0)$ exist''. That %definition is beyond the scope of this text. You can learn more by plugging 
%``differentiability  several  variables'' (without the quotes) into your %favourite search engine. The definition is designed to rule out weirdness like %that illustrated in Example \ref{eg partials DDD}.}. 
And there are many notations for them.
...
the number two. There are obvious variants for any number of variables. 
For example,
...
\begin{itemize}
\item 
the left hand side, $\pdiff{F}{s}$, has units of grams per second.
\item
...
\end{align*}
to the blue curve at the point  $\big(x_0\,,\,y_0\,,\,f(x_0,y_0)\big)$. 
...
(provided $G(x_0,y_0,z_0)=K$ and $\vnabla G(x_0,y_0,z_0)$ is nonzero).}
...
\begin{defn}\label{def_differential}
\begin{enumerate}[(a)]
\item 
...
of $\vr(c)$ is smaller than $1$ and the $y$ component of $\vr(c)$ is bigger
than $1$. So
\begin{align*}
...
\vnabla f(0,1) = f_x(0,1)\,\hi + f_y(0,1)\,\hj 
               = e\,\hi + 2e\,\hj
\end{equation*}
and the directional derivative in the direction $-\hi + \hj$ is
\begin{equation*}
...
%               = 4\,\hi + \hj + 4\,\hk
\end{equation*}
and the directional derivative in the direction $\llt 1\,,\,0\,,\,1\rgt$ is
\begin{equation*}
...
time $t$ is $x=z-ct$  (i.e. move to the left with speed $c$) you always 
...
$x$-axis was chosen so that $U(z)=\max_x U(x)$.
...
The above argument that leads to the solution $w(x,t)=U(x+ct)$
...
The reason that we have called it $\widetilde V$ instead of $V$ will become
evident shortly.
\item
...
subset $R$ of $\bbbr^2$, that $(a,b)$ is a point of $R$ that is not on the boundary of $R$, and that $f$ has a local maximum at $(a,b)$.
...
of such functions, we only need to consider their critical and singular
...
$\sin\theta$ or $x$ is zero the area $A(x,\theta)$ will also  be zero,
...
that the first equation forces $\cos\theta=-\frac{1}{2x}(100-4x)$
...
\end{itemize}
Our problem is to find the $m$ and $b$ that minimize $E(m,b)$. 
...
\end{center}
\end{efig}
The figure below shows some level curves of $f$. Observe from the level 
curves that
\begin{itemize}
\item
...
absolute maximum or minimum. We'll now consider how to find the 
...
Theorem \ref{thm stationary point}.}
   ...
   \item $(a,b)$ is in the interior of the set and  $f_x(a,b)$ or $f_y(a,b)$
        does not exist or
   \item $(a,b)$ is a boundary\footnote{It should be intuitively obvious 
...
Our first job is to find the equation of the line that contains 
$(0,2)$ and $(1,0)$. By way of review, we'll find the equation using three 
different methods.
\begin{itemize}\itemsep1pt \parskip0pt \parsep0pt
\item \emph{Method 1:}\ \ \ 
 You (probably) learned in high school that any line in 
the $xy$-plane\footnote{To be picky, any line in the $xy$-plane that is not parallel to the $y$ axis.}
...
\end{equation*}
\end{itemize}
By any of these three methods\footnote{In the third method, $x$ has just been renamed to $t$.}, we have that the side of the triangle that 
...
derivatives and that $\vnabla g(x,y,z)\ne\vZero$ and that the surface 
$g(x,y,z)=0$ is bounded\footnote{The condition that the surface $g(x,y,z)=0$ be bounded
is there to ensure that $f$ actually has maximum and minimum values on the surface
$g(x,y,z)=0$. To see what can go wrong when the surface is not bounded, observe that if $f(x,y,z)=z$ and $g(x,y,z)=x$, then $f$ takes all real values on the surface $g=0$. To learn more about this issue, plug ``extreme value theorem'' into the search engine of your choice.}, you
\begin{enumerate}
...
Find the rectangle of largest area (with sides parallel to the coordinate
...
in $\cT$. So $(*)$ says that $\vnabla f(a,b,c)$ must be in $\cT^\perp$.
...
+\mu\vnabla h(a,b,c)$  form a plane. Thus we have found all vectors
