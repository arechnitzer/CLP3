...
public class OE16D_8e extends Template {

     static {templateClass = new OE16D_8e() ; }
...
//          fEnv("useColorPs", "true") ;
          ...
          canvas.append( new fPsWrite(Red+"\n")) ;
               ...
               double Y = 2.0 ;
               fEnv("lineWidth", 1.0) ;
               Y=2; fCurve(new interSectY(proj,Y), 0,Math.sqrt(3-Y),OPEN)  ;
               Y=1.5; fCurve(new interSectY(proj,Y), 0,Math.sqrt(3-Y),OPEN)  ;
               Y=1; fCurve(new interSectY(proj,Y), 0,Math.sqrt(3-Y),OPEN)  ;
               Y=0.5; fCurve(new interSectY(proj,Y), 0,Math.sqrt(3-Y),OPEN)  ;
               Y=0; fCurve(new interSectY(proj,Y), 0,Math.sqrt(3-Y),OPEN)  ;
               fCurve(new interSect(proj),1,Math.sqrt(3),OPEN)  ;
               fLine3d(Math.sqrt(ZM),0,ZM,  0,0,ZM) ;
               fLine3d(0,0,0,  0,YM,0) ;
               fLine3d(0,0,ZM,  0,YM,3-YM) ;
               fLine3d(0,YM,3-YM,  0,YM,0) ;
               fLine3d(0,YM,3-YM,  1,YM,3-YM) ;

               fTeXlabel3d(1,-0.07,0.75, "br", "$z=x^2$") ;
               fTeXlabel3d(0,1.1,2, "bl", "$y+z=3$") ;
               fTeXlabel3d(0,2.1,0.5, "cl", "$y=2$") ;

          fEnv("psFillGray", 0.0) ;
               fDisk3d(Math.sqrt(3),0,3, rr, rr) ;
               fTeXlabel3d(Math.sqrt(3),-0.05,3, "cr", "$(\\sqrt{3},0,3)$") ;
               fDisk3d(0,0,3, rr, rr) ;
               fTeXlabel3d(0,0.1,3.05, "cl", "$(0,0,3)$") ;
               fDisk3d(0,2,1, rr, rr) ;
               fTeXlabel3d(0,2.1,1, "cl", "$(0,2,1)$") ;
               fDisk3d(1,2,1, rr, rr) ;
          canvas.append( new fPsWrite("0.0 setgray\n")) ;
          fEnv("lineWidth", 0.5) ;
          fEnv("headHalfWidth", 2*1.2) ;
          fEnv("headLength", 6*1.2) ;
               fArrow3d(1,2.5,0, 1,2,1) ;
               fTeXlabel3d(1,2.55,0, "cl", "$(1,2,1)$") ;
...
class interSectY implements S2V {

     double[][] proj = { {1, 0, 0},   { 0,  1, 0}   };
     double Y = 0 ;

   public interSectY(double[][] proj, double Y) {
        this.proj = proj ;
        this.Y = Y ;
   }
