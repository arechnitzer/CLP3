...
public class partialTotalX extends Template {

     static {templateClass = new partialTotalX() ; }
...
//          x1 = Math.sin(Math.PI*(1+al/180)) ;
          ...
          fTeXlabel3d(xm+1,-1,-1, "tr", "$x$") ;

          double xh, yh, xv, yv, zv, th, phi, xpsi, ypsi ;
          double y1, y2, y3, x1, x2, x3, z1, z2, z3, z4, r3, r4 ;


          fEnv("lineWidth", 0.5) ;
          canvas.append( new fPsWrite("0.7 setgray\n")) ;
          x1 = 0.75*r ; 
            z1 = Math.sqrt(r*r-x1*x1);
            fCurve(new circ3d(proj, x1,0,0, 0,0,z1, 0,z1,0),0,90,OPEN)  ;
            fLine3d(x1,0,0, x1,0,z1) ;
            fLine3d(x1,0,0, x1,z1,0) ;
          x2 = 0.6*r ; 
            z2 = Math.sqrt(r*r-x2*x2);
            fCurve(new circ3d(proj, x2,0,0, 0,0,z2, 0,z2,0),0,90,OPEN)  ;
            fLine3d(x2,0,0, x2,0,z2) ;
            fLine3d(x2,0,0, x2,z2,0) ;


          z3 = z1 ;
            x3 = x2 ;
            y3 = Math.sqrt(r*r-x3*x3 - z3*z3);
            r3 = Math.sqrt(r*r-y3*y3);
            canvas.append( new fPsWrite(Orange+"\n")) ;
            fEnv("lineWidth", 1.5) ;
            fCurve(new circ3d(proj, 0,y3,0, 0,0,r3, r3,0,0),0,90,OPEN)  ;
            canvas.append( new fPsWrite(Black+"\n")) ;
            canvas.append( new fPsWrite("0.7 setgray\n")) ;
            fEnv("lineWidth", 0.5) ;
            fLine3d(0,y3,0, r3,y3,0) ;
            fLine3d(0,y3,0,  0,y3,r3) ;


            z4 = Math.sqrt(r*r-x1*x1-y3*y3);
            r4 = Math.sqrt(x1*x1+y3*y3) ;
            canvas.append( new fPsWrite(Pink+"\n")) ;
            fEnv("lineWidth", 1.5) ;
            fCurve(new circ3d(proj,   0,0,z4, r4),0,90,OPEN)  ;
            canvas.append( new fPsWrite(Black+"\n")) ;
            fEnv("lineWidth", 0.5) ;
            canvas.append( new fPsWrite("0.7 setgray\n")) ;
            fLine3d(0,0,z4, r4,0,z4) ;
            fLine3d(0,0,z4, 0,r4,z4) ;

          y1 = Math.sqrt(r*r-z1*z1) ;
            canvas.append( new fPsWrite(Blue+"\n")) ;
            fEnv("lineWidth", 1.5) ;
            fCurve(new circ3d(proj,   0,0,z1, y1),0,90,OPEN)  ;
            fEnv("lineWidth", 0.5) ;
            canvas.append( new fPsWrite("0.7 setgray\n")) ;


            fLine3d(0,0,z1, y1,0,z1) ;
            fLine3d(0,0,z1, 0,y1,z1) ;
            ...
            fDisk3d(x1,0,z1, rr, rr, FILLED) ;
            fDisk3d(x1,y3,z4, rr, rr, FILLED) ;
          ...
          fEnv("lineWidth", 0.5) ;
          fArrow3d(0.5*x1, 0, z1+38, 0.3*x1, 0, z1) ;
          fTeXlabel3d(0.5*x1, 0, z1+38, "bc", "$z=z_1$") ; 
          fArrow3d(0, 2.5*y3, z4+25, 0, 1.4*y3, z4) ;
          fTeXlabel3d(0, 2.5*y3-10, z4+23, "bl", "$z=z_2$") ; 
          fArrow3d(1.8*x1, 0, 0.5*z4, x2, 0, 0.5*z4) ;
          fTeXlabel3d(1.7*x1, 0, 0.5*z4-2, "cr", "$x=x_1$") ; 
          fArrow3d(1.8*x1, y3, 0, x1, 0.4*y3, 0) ;
          fTeXlabel3d(1.8*x1, y3, -2, "tc", "$x=x_2$") ; 

          fArrow3d(0, y3, 1.2*r,  0, 0.5*y3, 0.99*r) ;
          fTeXlabel3d(0, y3, 1.2*r-2, "bl", "$z=f(x,y)$") ; 

          fArrow3d(0, 1.7*y3, 0.95*r,  0.55*x3, 1.4*y3, z1) ;
          fTeXlabel3d(0, 1.7*y3, 0.95*r, "bl", "$y=y(x),\\,z=z_1$") ; 

          fArrow3d(x3-6, -5, 0.95*r,  x3,y3-1.5,z3+1.5) ;
          fTeXlabel3d(x3-6, -5, 0.95*r, "br", "$P_0$") ; 
          fTeXlabel3d(x1,-4,z1+2, "cr", "$P_1$") ; 
          fArrow3d(1.6*x1, 2.3*y3, 3,  x1,y3+1.5,z4-1.5) ;
          fTeXlabel3d(1.6*x1, 2.3*y3-4, 0, "tl", "$P_2$") ; 
