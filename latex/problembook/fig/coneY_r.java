...
public class coneY_r extends Template {

     static {templateClass = new coneY_r() ; }
...
//          x1 = Math.sin(Math.PI*(1+al/180)) ;
          double y0 = 4 ; double X = 0 ; double Y = 0 ; double Z = 0 ;
...
//          fTeXlabel3d(xm,-0.05,-0.05, "cr", "$x$") ;
          ...
          fEnv("lineWidth", 1.5) ;
          double th = 6 ; double thRad = th*Math.PI/180 ;
          double s = Math.sin(thRad) ; double c = Math.cos(thRad) ; 
          fLine3d(y0*s,y0*c,y0,  -y0*s,-y0*c, -y0) ;


          th = 17 ;  thRad = th*Math.PI/180 ;
          s = Math.sin(thRad) ; c = Math.cos(thRad) ; 
          double f = 0.90 ; double ff = 0.92 ;
          fLine3d(f*y0*s,f*y0*c,-f*y0,  -ff*y0*s,-ff*y0*c, ff*y0) ;


          fEnv("psFillGray", 0.8) ;
          y0 = 4 ; X = y0 ; Z = y0 ;
          fCurve(new circ3d(proj,   0,y0,0, X,0,0, 0,0,Z),
                 0,360,FILLED) ;
          fCurve(new circ3d(proj,   0,y0,0, X,0,0, 0,0,Z),
                 0,360,OPEN) ;
